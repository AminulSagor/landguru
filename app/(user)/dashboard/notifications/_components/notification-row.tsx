...
import { AlertTriangle, Hammer, FileText, CalendarCheck, BadgeCheck, Tag } from "lucide-react";
...
function getMeta(n: NotificationItem) {
  ...
  if (n.title.toLowerCase().includes("offer")) {
    return { icon: <AlertTriangle size={18} />, iconBg: "bg-red-100 text-red-600", leftBar };
  }
  if (n.title.toLowerCase().includes("quoted")) {
    return { icon: <Hammer size={18} />, iconBg: "bg-red-100 text-red-600", leftBar };
  }
  if (n.title.toLowerCase().includes("service")) {
    return { icon: <FileText size={18} />, iconBg: "bg-blue-100 text-blue-600", leftBar };
  }
  if (n.title.toLowerCase().includes("appointment")) {
    return { icon: <CalendarCheck size={18} />, iconBg: "bg-emerald-100 text-emerald-700", leftBar };
  }
  if (n.title.toLowerCase().includes("verified")) {
    return { icon: <BadgeCheck size={18} />, iconBg: "bg-gray-100 text-gray-700", leftBar };
  }
  return { icon: <Tag size={18} />, iconBg: "bg-purple-100 text-purple-700", leftBar };
}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-4 p-5 pl-6">
        {/* Icon bubble */}
        <div className={cx("h-10 w-10 rounded-full flex items-center justify-center", meta.iconBg)}>
          {meta.icon}
        </div>
